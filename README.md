...
- [x] Stage 2 complete: Windows Server 2022 VM (win-client) created; host-only network (192.168.56.0/24) between both VMs; Wazuh agent Active; VirtualBox Guest Additions installed; Sysmon v15.22 installed with the Neo23x0 config; Sysmon-to-Wazuh integration confirmed working after diagnosing and fixing a malformed channel path (INV-002); SC-002 (base64 PowerShell, T1059.001) detected cleanly (INV-003); SC-003 (scheduled task, T1053.005) found a real detection gap, schtasks.exe isn't logged by the current Sysmon config (INV-004). Stage 3 (more scenarios, custom detections, fixing this gap) is next.
