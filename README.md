   ...
   - [x] Stage 2 in progress: Windows Server 2022 VM (win-client) created; host-only network (192.168.56.0/24) between both VMs; Wazuh agent installed and Active; Sysmon v15.22 installed with the Neo23x0 config, confirmed logging events (Process Create, File Created) in Event Viewer. Still to do: confirm Wazuh receives Sysmon events, run and detect a first Windows scenario.
